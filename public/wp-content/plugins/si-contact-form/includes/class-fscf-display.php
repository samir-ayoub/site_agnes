...
<div ' . self::get_this_css('field_div_style') . '>'
	...
	static function display_recaptcha() {
		// this function adds the recaptcha to the contact form
             self::$add_recaptcha_script = 1; // used in util to decide to print js
             $theme = 'light';
             if (self::$form_options['recaptcha_dark'] == 'true')
                 $theme = 'dark';

             $size = 'normal';
             if (self::$form_options['captcha_small'] == 'true')
                 $size = 'compact';
               // not compatible with multiforms
/*             $string = "\n<div " . self::get_this_css('title_style') . ">\n</div>\n" .  self::echo_if_error( 'captcha' )  .

'<div class="g-recaptcha" data-size="'.$size.'" data-theme="'.$theme.'" data-sitekey="'.esc_attr(self::$global_options['recaptcha_public_key']).'"></div>
 ...
 ';*/
              ...
              $string = "\n<div " . self::get_this_css('title_style') . ">\n</div>\n" .  self::echo_if_error( 'captcha' )  . '<div id="fscf-recaptcha' . self::$form_id_num . '"></div>';
