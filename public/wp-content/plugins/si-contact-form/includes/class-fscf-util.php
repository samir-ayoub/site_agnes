...
';
       ...
       foreach ( FSCF_Display::$add_recaptcha_js_array as $v ) {
          ...
          $pieces = explode("||", $v);
		  $string .= "
grecaptcha.render('fscf-recaptcha".esc_js($pieces[0])."', {'sitekey' : '".esc_js($pieces[1])."', 'size' : '".esc_js($pieces[2])."', 'theme' : '".esc_js($pieces[3])."'});
";
	   }
...
<?php
   ...
   static function make_fscf_script_async( $tag, $handle, $src ) {
     // google recaptcha js needs to be loded async
     // make wordpress load js async, but only for this script handle
