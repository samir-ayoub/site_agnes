        ...
        if ( $captcha_enabled == 'recaptcha' ) {
		   // validate the recaptcha now
            ...
            if( isset( $_POST['g-recaptcha-response'] ) ) {
                 ...
                 $response = json_decode(wp_remote_retrieve_body( wp_remote_get( "https://www.google.com/recaptcha/api/siteverify?secret=".sanitize_text_field(self::$global_options['recaptcha_secret_key'])."&response=" .sanitize_text_field($_POST['g-recaptcha-response'])."&remoteip=" . $ip) ), true );
               if( $response["success"] ) {
                   // ok, can continue
               } else {
